    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new FirestoreRecyclerAdapter<ViewModel, HarvestViewHolder>(options) {
            ...
            protected void onBindViewHolder(@NonNull HarvestViewHolder holder, int position, @NonNull ViewModel model) {
                String period, cropType, production, region, area;
                period = "Period:- " + model.getMonth() + " " + model.getYear();
                cropType = "Crop Type:- " + model.getCropType();
                production = "Production:- " + model.getProduction() + " Tonnes";
                region = "Region:- " + model.getRegion();
                area = "Area:- " + model.getArea() + " Acres";

                holder.mPeriod.setText(period);
                holder.mCrop_type.setText(cropType);
                holder.mProduction.setText(production);
